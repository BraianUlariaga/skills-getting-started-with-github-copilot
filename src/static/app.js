document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
        ...
        activityCard.innerHTML = `
          ...
          <ul>
            ${participants.length > 0 
              ? participants.map(email => `<li>${email} <button class="remove-btn" data-activity="${name}" data-email="${email}">×</button></li>`).join('')
      ...
      console.log('Options in selector:', activitySelect.querySelectorAll('option').length);
    } catch (error) {
